...
- 按照 `PLUGIN_CLIENT.spec.md` 重构 dsh-tauri-ui 插件客户端半区（Browser Runtime）的架构。
- 按照 `PLUGIN_HOST.spec.md` 重构 dsh-tauri-ui 插件宿主端半区（Host Runtime）的架构。
- 按照 `PLUGIN_HOST_SERVICE.spec.md` 重构 dsh-tauri-ui 插件宿主端领域服务半区（Host Service Runtime）的架构。

# Agent - 2

- 按照 `PLUGIN_CLIENT.spec.md` 重构 dsh-tauri-worktree 插件客户端半区（Browser Runtime）的架构。
...
- 按照 `PLUGIN_HOST_SERVICE.spec.md` 重构 dsh-tauri-turnrewind 插件宿主端领域服务半区（Host Service Runtime）的架构。
