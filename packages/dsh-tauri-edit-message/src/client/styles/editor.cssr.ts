/**
 * client/styles/editor.cssr.ts — 编辑面板样式（BEM 块 `edit-message`）。
 *
 * 只作用于插件自己注入的节点；官方气泡 / 操作行的外观保持内核原样。
 * 颜色一律走共享设计 token（`theme.ts` 的同一批变量），亮色与暗色主题自动适配。
 *
 * 全部用 bem 助手（`b/e/m`），不与 `cssr.c` 混用 —— 两者产生的 CNode 不能互为子节点。
 *
 * bem 的嵌套是**后代**选择器：块里的元素会渲染成
 * `.dshp-edit-message .dshp-edit-message__edit-btn`。因此每个注入节点的外层都必须带块类
 * `dshp-edit-message`（见 `dom/message-patch.tsx`）—— 之前 Edit 按钮退回浏览器默认样式，
 * 就是因为它被追加进**官方**操作行、外层没有块元素。
 */

import { cssr, styles as sharedStyles } from 'dsh-tauri-ui/client'

const { c, bem: { b, e, m } } = cssr
const { primary, secondary, tertiary, borderL2, modulePlatform, hover, error, primaryFill, primaryHover, primaryFg } = sharedStyles

export default b('edit-message', {}, [
  // 追加在官方操作行里的 Edit 图标按钮：外观与官方 `.xzv4MW_action` 对齐，
  // 这里只兜底尺寸，避免官方类名换代时按钮塌陷。
  e('edit-btn', {
    width: '28px',
    height: '28px',
    display: 'inline-flex',
    alignItems: 'center',
    justifyContent: 'center',
    padding: '6px',
    border: 'none',
    borderRadius: '28px',
    background: 'transparent',
    color: tertiary,
    cursor: 'pointer',
  }, [
    // 与官方 `.xzv4MW_action` 同款 hover：淡底 + 提亮前景。
    // `c` 只用来表达伪类/组合选择器（bem 助手没有这种能力），仍是同一个 CNode 树。
    c('&:hover', { background: hover, color: secondary }),
    c('& svg', { width: '15px', height: '15px' }),
  ]),

  // 官方没有操作行（echo / pending 行）时自建的一条操作行。
  e('own-actions', {
    display: 'flex',
    alignItems: 'center',
    justifyContent: 'flex-end',
    gap: '8px',
  }),

  // 编辑面板：宽 100%、圆角、灰底；操作区在右下角。
  e('editor', {
    width: '100%',
    boxSizing: 'border-box',
    background: modulePlatform,
    border: `1px solid ${borderL2}`,
    borderRadius: '8px',
    padding: '10px 12px 8px',
    display: 'flex',
    flexDirection: 'column',
    gap: '8px',
  }),
  e('textarea', {
    width: '100%',
    minHeight: '46px',
    maxHeight: '320px',
    resize: 'vertical',
    border: '0',
    outline: 'none',
    background: 'transparent',
    color: primary,
    font: 'inherit',
    fontSize: 'var(--dsh-content-font-size,14px)',
    lineHeight: 'calc(22px + var(--dsh-content-font-delta,0px))',
  }),
  e('actions', {
    display: 'flex',
    justifyContent: 'flex-end',
    gap: '8px',
  }),
  // 次级按钮：「取消」。
  e('btn', {
    padding: '4px 14px',
    borderRadius: '8px',
    border: `1px solid ${borderL2}`,
    background: 'var(--dsw-alias-bg-layer-1,var(--dsw-alias-bg-base))',
    font: 'inherit',
    fontSize: '13px',
    color: primary,
    cursor: 'pointer',
  }, [
    c('&:hover', { background: hover }),
    c('&[disabled]', { opacity: '.5', cursor: 'default' }),
    // 主按钮：「发送」——黑底白字（前景色必须显式指定，否则继承 label-primary 会黑底黑字）。
    m('primary', {
      background: primaryFill,
      borderColor: 'transparent',
      color: primaryFg,
    }, [
      c('&:hover', { background: primaryHover, color: primaryFg }),
    ]),
  ]),
  e('error', { fontSize: '12px', color: error }),
])
