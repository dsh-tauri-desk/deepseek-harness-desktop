...
setlocal
set "APP_DIR={app_dir}"
rem Use bundled MinGit only when no usable system Git is available.
git --version >nul 2>nul
if errorlevel 1 if exist "%APP_DIR%\dependencies\git\cmd\git.exe" set "PATH=%APP_DIR%\dependencies\git\cmd;%PATH%"
{user_dsh}
set "DSH_BIN={dsh_bin}"
...
$appDir = '{app_dir}'
$dshBin = '{dsh_bin}'
# Use bundled MinGit only when no usable system Git is available.
$systemGitWorks = $false
try {{
    & git --version *> $null
    $systemGitWorks = ($LASTEXITCODE -eq 0)
}} catch {{}}
...
mod tests {
    ...
    fn windows_shims_configure_bundled_git_without_shadowing_system_git() {
        ...
        for content in [&dsh_cmd, &pnpm_cmd] {
            assert!(content.contains("git --version >nul 2>nul"));
            assert!(content.contains(r"dependencies\git\cmd\git.exe"));
            assert!(content.contains(r"dependencies\git\cmd;%PATH%"));
            let system_probe = content.find("git --version").expect("system Git probe");
        ...
        for content in [&dsh_ps1, &pnpm_ps1] {
            assert!(content.contains("& git --version *> $null"));
