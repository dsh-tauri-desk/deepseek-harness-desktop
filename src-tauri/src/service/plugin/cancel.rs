...
//! - Windows：使用安装启动时登记的根进程 PID 结束进程树（`taskkill /T /F`）。
...
pub async fn cancel(app_handle: &AppHandle) {
    // Unix：结束注册 PID 对应的安装进程组；Windows：结束登记的根进程树。
    #[cfg(not(windows))]
    {
    ...
    {
        if let Some(pid) = super::process::active_plugin_pid() {
            log::info!("Cancelling dsh plugin install process tree led by pid {pid}");
